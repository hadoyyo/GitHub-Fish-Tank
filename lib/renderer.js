...
function generateAquariumSVG(languageStats, options = {}) {
  ...
  `;
    ...
    if (showLegend) {
      ...
      <g transform="translate(5, ${20 + index * 28})">
        ...
        <text x="30" y="14" font-size="15" fill="#ffffff" font-family="monospace">${displayName}: ${percentage.toFixed(1)}%</text>
      </g>
    `);
    }
  });
  ...
  const glassFrame = `
    <rect x="0" y="0" width="${width}" height="${height}" fill="none" stroke="${frameColor}" stroke-width="8" rx="${borderRadius}" ry="${borderRadius}" />
    <rect x="4" y="4" width="${width-8}" height="${height-8}" fill="none" stroke="${frameHighlightColor}" stroke-width="2" opacity="0.3" rx="${borderRadius-4}" ry="${borderRadius-4}" />
      ...
      <g clip-path="url(#roundedCorners)">
        ...
        ${showLegend ? `
          <g transform="translate(${width - 200}, 0)"> 
            <rect x="0" y="10" width="190" height="${legendItems.length * 28 + 15}" fill="rgba(0, 0, 0, 0.6)" rx="8" />
            <g font-family="monospace" font-size="15" fill="#ffffff">
              ${legendItems.join('\n')}
            </g>
          </g>
        ` : ''}
